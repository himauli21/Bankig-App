...
//	
	 ...
	 *   - 1 CAD = 0.75 USD - 1 EUR = 1.14 USD
	 *   - 1 BRL = 0.27 USD - 1 CAD = 0.75 USd
	 *   - So:   		10 CAD = 7.5 USD - 10 EUR = 11.4 USD
	 *   - And: 		7.5 USD = 27.78 BRL - 11.4 USD = 8.55 CAD
	 *   - Therefore:   10 CAD = 27.78 BRL - 10 EUR = 8.55 CAD
	...
	*/
	public double valueInThisCurrency(double amount, Currency othercurrency) {
		...
		double thisCurrencyRate = getRate(); 
		
		setRate(othercurrency.getRate()); 
		
		Double valueOfOtherCurrencyInUSD = valueInUSD(amount); 
		
		Double valueOfThisCurrencyInUSD = valueOfOtherCurrencyInUSD / thisCurrencyRate ;
		
		
		return round(valueOfThisCurrencyInUSD,2);
